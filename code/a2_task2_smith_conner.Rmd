...
The parameter estimates were used to provide initial guesses for the model, $ W = a(SVL)^b $. From the calculations here, we see that $a$ = `r round(tidy_nls$estimate[1], 4)` and $b$ = `r round(tidy_nls$estimate[2], 2)`. 
...
The parameter estimates were used to provide initial guesses for the Western Whiptail-specific model, $ W_{whip} = a(SVL)^b $. From the calculations here, we see that $a$ = `r round(tidy_whip$estimate[1], 4)` and $b$ = `r round(tidy_whip$estimate[2], 2)`. We see that the $b$ parameter is slightly lower for this subset but the $a$
 parameter is slightly higher. 
 
```{r}
# Plot the NLS for the full data against the NLS for the Whiptail subset, starting by calculating RMSE for both. 
